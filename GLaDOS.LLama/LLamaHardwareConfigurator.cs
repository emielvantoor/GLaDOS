...
public static class LLamaHardwareConfigurator
{
    ...
    public static void Configure(IConfiguration configuration)
    {
        ...
        var hardwareMode = configuration["GLaDOS:HardwareMode"] ?? "CPU";
        ...
        if (_useGpu)
        {
                ...
                if (chosenGpuDir != "/usr/lib")
                {
                    ...
                    if (!File.Exists(targetCpuLib) && File.Exists(sourceCpuLib))
                    {
                        try
                        {
                            ...
                            Console.WriteLine($"[GLaDOS] Linker-fix: libggml-cpu.so succesvol gekopieerd naar {gpuTechnology} map.");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"[GLaDOS] Waarschuwing: Kon libggml-cpu.so niet kopiëren: {ex.Message}");
                        }
                    }
                }
            }
            ...
            if (File.Exists(llamaPath))
            {
                ...
                Console.WriteLine($"[GLaDOS] GPU hardware gedetecteerd! Succesvol {gpuTechnology} geselecteerd.");
            }
            else
            {
                Console.WriteLine($"[GLaDOS] GPU binaries niet gevonden in {chosenGpuDir}. Fallback naar CPU.");
    ...
    private static void ConfigureCpuFallback(string nativeRootDir, string libExtension, string prefix)
    {
        ...
        if (File.Exists(llamaPath))
        {
            var finalBackend = File.Exists(ggmlPath) ? ggmlPath : null;
            NativeLibraryConfig.Instance.WithLibrary(llamaPath, finalBackend);
            Console.WriteLine($"[GLaDOS] CPU AVX2 ({prefix}llama{libExtension}) geselecteerd.");
        }
        else
        {
            ...
            if (File.Exists("/usr/lib/libllama.so") && prefix == "lib")
            {
                ...
                Console.WriteLine($"[GLaDOS] Geselecteerd via Arch Linux systeem-bibliotheek (/usr/lib/libllama.so).");
            }
            else
            {
                NativeLibraryConfig.Instance.WithLibrary(prefix == "lib" ? "libllama.so" : "llama.dll", null);
                Console.WriteLine($"[GLaDOS] Lokale CPU binaries niet gevonden, gebruik gemaakt van standaard-fallback.");
            }
        }
    }
    ...
    {
        var modelPath = configuration["GLaDOS:ModelPath"] ??
    ...
    public static ModelParams CreateOptimizedParameters(IConfiguration configuration, string modelPath)
    {
        ...
        var contextSize = uint.TryParse(configuration["GLaDOS:ContextSize"], out var size) ? size : 3072;
