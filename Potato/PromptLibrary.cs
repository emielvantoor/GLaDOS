...
internal static class PromptLibrary
{
    ...
    private const string DefaultToolInstructions =
        ...
        "For README or project documentation generation, do not write after only a top-level listing. First inspect the top-level repository structure with ListFiles using recursive=false, use SearchFiles to find relevant source/config/project/documentation files and folders, use targeted non-recursive ListFiles calls for relevant subdirectories, ignore hidden metadata folders, read or summarize representative files, include a short introductory overview before listing folders or key files, and ensure the document does not repeat or duplicate the same content across sections.\n" +
        "For project changes, do not edit after only a directory listing. First use ListFiles with recursive=false for a top-level overview, use SearchFiles when looking for known file names or extensions, use SearchFileContents when looking for known text, symbols, or code snippets inside files, use targeted non-recursive ListFiles calls for relevant subdirectories, summarize likely relevant files with SummarizeFilePurpose, read the exact files that own the requested behavior, then edit only that path.\n" +
